...
To pit these two accounts against each other, they led American English listeners read sentences and used a computer mouse to indicate whether each sentence was true or false. In three experiments, the authors present data that suggest that when verifying sentences like "some scientists preregister their studies", average mouse paths initially move towards the true target and then changed direction mid-flight to select the false target. This effect is modulate by the feedback that listeners receive in a critical training phase. One groupd of listeners were trained to interpret the critical sentences as possibly referring to all elephants, another group was trained to interpret these sentences as referring only to a subset of elephants but not all. 

The present study, conceptually replicates Experiment 1 of the original study with German listeners. We are only looking at the critical condition and we are only measuring the curvature of the trajectories as operationalized by the area-under-the-curve measure (see below for details). Using clustering algorhythms, we test whether these curvature effects are simple artifacts of averaging over two categorically discrete movement trajectories. 
